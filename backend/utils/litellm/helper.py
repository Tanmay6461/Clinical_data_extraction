...
- **Include only events in `upcoming_milestones` that are still in the future relative to today.**
...
Return a valid JSON object where each key is the drug name (e.g., "AMVUTTRA", "WVE-120101") and its value contains the summarized fields. Do not wrap all drugs inside a single \"drugs\" key.
...
{{
  "AMVUTTRA": {{
    ...
    "preclinical_data": ["In mdx23 mice, four weekly doses induced up to 20% exon skipping and restored dystrophin protein by 70-90%."],
    "clinical_trials": [
