...
import json
from utils.gcp.cloud_storage import upload_file, read_json_from_gcs, write_json_to_gcs
from io import BytesIO, StringIO
from google.oauth2 import service_account
from utils.prompts import prompt_get_drug_names, prompt_group_drug_names, final_drug_info_schema
from utils.litellm.llm import llm
from collections import defaultdict
from concurrent.futures import ThreadPoolExecutor, as_completed
from jsonschema import validate, ValidationError
import shutil
from utils.hleper import get_gcp_credentials_from_airflow
...
class SECFilingExtractor:
    ...
    def __init__(self, ticker, email, output_dir="sec_extracts"):
        ...
        self.api_key = Variable.get("OPENAI_API_KEY")
        self.client = OpenAI(api_key=self.api_key)

        self.credentials = get_gcp_credentials_from_airflow("google_cloud_default")
    ...
    def process_filing(self, filing, form_type):
        ...
        filing_year = filing['filingDate'].split('-')[0]
        ...
        raw_text = content.getvalue()
        chunk_size = 150000
        all_programs = []

        for i in range(0, len(raw_text), chunk_size):
            chunk = raw_text[i:i + chunk_size]
            formatted_prompt = prompt_get_drug_names.format(ticker=self.ticker, content=chunk)
            ...
            if not llm_response or "response" not in llm_response:
                print(f"[Chunk {i//chunk_size+1}] LLM response missing or failed.")
                continue
            ...
            try:
                ...
                all_programs.extend(parsed.get("programs", []))
            except json.JSONDecodeError as e:
                print(f"[Chunk {i//chunk_size+1}] JSON decode error: {e}")
                continue

        final_response = {"programs": all_programs}
    ...
    def process_all_filings(self, form_types=("8-K", "10-K", "10-Q"), max_workers=6, override_filings=None):
        try:
            if override_filings:
            ...
            def handle_filing(filing):
                try:
                    ...
                    print(f"Processing filing: {filing['accessionNumber']}")
                    data = self.process_filing(filing, form_type)
                    print(f"Finished processing: {filing['accessionNumber']}")
            ...
            try:
                ...
                formatted_prompt =  prompt_group_drug_names.format(
                    content=drugs_info_json
                    )               
                llm_names_grouping = llm(
                    model="gpt-4.1-mini",
                    prompt=formatted_prompt, 
                    client=self.client
                )  
                             
                raw_response = llm_names_grouping['response'].strip()
            ...
            final_merge = self.merge_all_drug_data(partial_path, raw_response)
